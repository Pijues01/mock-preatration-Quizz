                                    ...
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            <div class="text-xs font-weight-bold text-primary text-uppercase mb-1">
                                                No of Exams are public</div>
                                            <div class="h5 mb-0 font-weight-bold text-gray-800">
                                                {{ $public_exam }}/{{ $exam_count }}</div>
                                        </div>
                                        <div class="col-auto">
                                ...
                                <tbody>
                                    @if (count($allexam) > 0)
                                        @php
                                            $count = 1;
                                        @endphp
                                        @foreach ($allexam as $key => $examdetails)
                                            <tr>
                                                <th scope="row">{{ $count++ }}</th>
                                                <td>{{ $examdetails->exam_name }}</td>
                                                <td>{{ $examdetails->no_student }}</td>
                                                <td>{{ $examdetails->passing_percentage }}</td>
                                                @if ($examdetails->active == '1' && $examdetails->public == '1' && $examdetails->end_date_time > date('Y-m-d H:i:s'))
                                                    <td><span class="badge badge-pill badge-success">Public</span></td>
                                                @elseif ($examdetails->active == '0')
                                                    <td><span class="badge badge-pill badge-danger">Deleted</span></td>
                                                @elseif ($examdetails->end_date_time < date('Y-m-d H:i:s'))
                                                    <td><span class="badge badge-pill badge-secondary">Expire</span>
                                                    </td>
                                                @else
                                                    <td><span class="badge badge-pill badge-dark">Private</span></td>
                                                @endif


                                                <td><a href="{{ url('examinfo/' . enc($examdetails->id)) }}"><button
                                                            type="button"
                                                            class="btn btn-info btn-sm">Info</button></a></td>
                                        @endforeach
                                    @else
                                        <tr>
